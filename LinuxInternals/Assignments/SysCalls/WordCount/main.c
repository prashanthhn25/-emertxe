 ...
 *	2. If more than one files passed, print individual count values and 
 *		file name + calculate the total of all values and print at the end.
 *	3. If no file are passed wc will read from std input till end of file(Ctrl + D) 
 *		then count lines, words and characters(bytes).
 *	4. Implement a word_count() function where you have to pass fd and 3 integer addresses 
 *		(pass by refference). Int word_count (int fd, int *lines, int *words, int *bytes);
 *	5. Word_count function will read from the fd and calculates lines, words and bytes, 
 *		then stores into respective addresses passed (don’t print values inside function).
 *	6. Main function will open the files in a loop and call word_count function depends 
 *		upon number of files passed. Print values after calling functions in main.
